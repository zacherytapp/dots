...
const SESSION_DIR = process.env.BRAINSTORM_DIR || '/tmp/grilling-visual';
const CONTENT_DIR = path.join(SESSION_DIR, 'content');
const STATE_DIR = path.join(SESSION_DIR, 'state');
const COMPANION_NAME = 'Grilling Visual Companion';
...
<html>
<head><meta charset="utf-8"><title>Grilling Visual Companion</title>
<style>
...
<body><!-- BRANDING --><h1>Grilling Visual Companion</h1>
...
function renderBranding(html) {
  const branding = '<div class="brand"><span class="brand-copy">' + COMPANION_NAME + '</span></div>';
  return html.split('<!-- BRANDING -->').join(branding);
}
